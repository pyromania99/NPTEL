...
def main():
    """Run automated tuning for INDI parameters"""
    parser = argparse.ArgumentParser(description="INDI Scale and Blend Ratio Tuning using Bayesian Optimization")
    parser.add_argument("--use-history", dest="use_history", action="store_true", help="Use prior tuning_results.json to warm start")
    parser.add_argument("--no-history", dest="use_history", action="store_false", help="Ignore history and start fresh", default=True)
    parser.add_argument("--max-seed-cost", type=float, default=None, help="Max cost from history to seed the optimizer")
    parser.add_argument("--results-file", type=str, default=None, help="Path to results DB JSON (defaults to flight_data/outputs/indi_tuning_results.json)")
    parser.add_argument("--trials", type=int, default=25, help="Number of optimization calls")
    ...
    # Use separate results file for INDI tuning to avoid conflicts
    default_results_file = f"{flight_data_dir}/outputs/indi_tuning_results.json"
    ...
    tuner = ControllerAutoTuner(
        ...
        results_file=args.results_file or default_results_file
    )
    # Ensure we survive until each trial prints the cost
    tuner.install_signal_handlers()
    
    print("=== INDI Scale and Blend Ratio Tuning ===")
    print("Testing INDI scale (0.1-1.0) and blend ratio (0.0-1.0)")
    print("History disabled by default for focused INDI testing")
    ...
    # Save results
    out_file = args.results_file or default_results_file
